<script>
    ...
    async function getDatasets () {
        ...
        try{
            ...
            const json = await res.json();
            if(dsname){
                datasets = (json.result||[]).filter(i=>i.name.toLowerCase()==decodeURI(dsname).toLowerCase());
            }
            else{
    ...
    function handleResize() {
        ...
        colHeight = (window.innerHeight-90)+"px";
        colpanelHeight=(window.innerHeight-drop_zone.offsetTop-61)+"px";
        const pg = parseInt((window.innerHeight-el.offsetTop-80)/36);
        if(pg>1) page_size = pg;
        else page_size = 10;
...
<div class="flex flex-row w-full" on:resize={handleResize}>
    <div  class="w-1/5 border-r shadow-md p-2 bg-gray-100" style="min-height:{colHeight};">
