<script>
    ...
    $:{
      ...
      if(reportObject!==null){
        ...
        let bodyW = report.inToPixels(reportObject.body.width||"5.6in");
        const pgH = calculateDimension(headerelements);
        const pgF = calculateDimension(footerelements);
        canvasMaxWidth=pgwidth-(lm+rm)-5;
        if(canvasWidth>canvasMaxWidth) canvasWidth=canvasMaxWidth;
        if(canvasWidth<bodyW) canvasWidth=bodyW;
        if(headerHeight<Math.max(pgH.height,minHeaderFooter)) headerHeight=Math.max(pgH.height,minHeaderFooter);
        if(footerHeight<Math.max(pgF.height,minHeaderFooter)) footerHeight=Math.max(pgF.height,minHeaderFooter);
        const dim = calculateDimension(bodyelements);
        const minH =Math.max(dim.height,minBodyHeight);
        if(bodyHeight<minH) bodyHeight=minH;
    ...
    $: showFilters = dataset.length>0 && selectedObjectType=="tablix" && !!selectedObject && report.shouldHaveFilter(reportObject.dataSets.dataSet[0]);
    $: previewHeight= (preview && headercontrols!==undefined && (window.innerHeight - headercontrols.offsetHeight)) || 0;

    report.getReportForEdit(reportId).then(result=>{
          reportInfo=Object.assign({},reportInfo, {
              id:result.id, 
              name: result.name, 
          ...
          canvasWidth = report.inToPixels(reportObject.body.width||"5.6in");
          headerHeight =report.cmToPixels(reportObject.page.pageHeader.height);
          footerHeight = report.cmToPixels(reportObject.page.pageFooter.height);
          bodyHeight = report.cmToPixels(reportObject.body.height);
          waiting=false;
    ...
    function makeElements(){
       const page=reportObject.page;
       const body=reportObject.body;
       if(page && page.pageHeader && page.pageHeader.reportItems!=null){
          makeSectionElements(page.pageHeader, "header");
          viewHeader=elements.filter(i=>i.section==="headersection").length>0;
       }
          
       makeSectionElements(body, "body");
       if(page && page.pageFooter && page.pageFooter.reportItems!=null ){
          makeSectionElements(page.pageFooter, "footer");
          viewFooter=elements.filter(i=>i.section==="footersection").length>0;
       }
       
    }
    function makeSectionElements(section, name){
        const sectionels=section.reportItems;
        const sectionName=`${name}section`;
        // tables
        for (let index = 0; index < sectionels.tablix.length; index++) {
          const data = sectionels.tablix[index];
          frames.push({
              translate: [0, 0],
          });
          let el={
              ...
              text:"Table",
              position: {
                x: report.cmToPixels(data.left), 
                y: report.cmToPixels(data.top), 
                height: report.cmToPixels(data.height)||50, 
                width: report.cmToPixels(data.width)
              },
              data: data
          };
        ...
        // textbox
        for (let index = 0; index < sectionels.textbox.length; index++) {
          const data = sectionels.textbox[index];
          frames.push({
              translate: [0, 0],
          });
          let el={
              ...
              position: {
                x: report.cmToPixels(data.left), 
                y: report.cmToPixels(data.top), 
                height: report.cmToPixels(data.height), 
                width: report.cmToPixels(data.width)
              },
              data: data
          };
        ...
        // line
        for (let index = 0; index < sectionels.line.length; index++) {
          const data = sectionels.line[index];
          frames.push({
              translate: [0, 0],
          });
          let el={
        ...
        // image
        for (let index = 0; index < sectionels.image.length; index++) {
          const data = sectionels.image[index];
          frames.push({
              translate: [0, 0],
          });
          let el={
        ...
        // rectangle
        for (let index = 0; index < sectionels.rectangle.length; index++) {
          const data = sectionels.rectangle[index];
          frames.push({
              translate: [0, 0],
          });
          let el={
    ...
    function holdermousemove(e){
        ...
        if(resizeContext==="vsz_designholder"){
            ...
            //reportObject.body.width=report.px2inFormat(canvasWidth);
        }
    ...
    */
    ...
    async function saveDraft(e){
        waiting=true;  
        await report.saveDraft(reportInfo.name, reportInfo.category,reportObject)
          .then(r=>{
            waiting=false;
          })
          .catch(error=>{
    ...
    async function doPublish(e){
      ...
      Swal.fire({
          ...
          input: 'text',
          inputValue: reportInfo.draft?'':reportInfo.category,
          html: `${reportInfo.draft?'Please enter category to publish':'Category name '}`,
          inputPlaceholder: "Type other than Drafts",
          buttonsStyling: false,
          showCancelButton: true,
          confirmButtonText: 'Publish',
          confirmButtonClass: "btn px-2 mx-2 bg-blue-500 text-white focus:outline-none",
          cancelButtonClass: "btn px-2 mx-2 bg-red-400 text-white focus:outline-none",
          icon: "info",
          preConfirm: (input)=>{
              if(input.trim()===''){
                Swal.showValidationMessage(
                  `You must enter category to publish.`
                );
              } else if(input.toLowerCase().trim()==='draft'||input.toLowerCase().trim()==='drafts'){
                Swal.showValidationMessage(
        ...
        if (result.isConfirmed) {
          waiting=true;
          reportInfo.category=result.value;
          await report.publish(reportInfo.name, reportInfo.category, reportObject)
                      .then(r=>{
                        reportInfo.draft=false;
                        waiting=false;
                        goBack();
                      })
                      .catch(error=>{
    ...
    function visibility_right_toolbar(e){
      hideRightTools=e.detail;
    }
</script>
...
<div  class="flex flex-col text-sm min-h-screen" on:resize={handleResize} 
    ...
    <div bind:this={headercontrols} class="p-1 bg-gray-100 flex flex-row items-center justify-between">
      <div class="flex flex-row">
        <svg on:click|preventDefault={goBack} class="w-8 v-8 p-1 mx-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
        </svg>
        <input type="text" bind:value={reportInfo.name} readonly class="text-xs w-64 p-2 hover:outline-none focus:outline-none focus:ring-1 focus:ring-indigo-400 rounded-sm"/>
      </div>
      <div class="flex flex-row">
      ...
      <div class="flex flex-row space-x-2 items-center justify-between">
        {#if reportInfo && reportInfo.draft || false }
        <div>
          <button 
              on:click|preventDefault={saveDraft}
              class="p-1 px-2 text-white bg-blue-500  border border-blue-600 rounded-sm hover:bg-green-500 focus:outline-none">
              Save Draft
          </button>
        </div>
        {/if}
        <div>
           <button on:click|preventDefault={doPublish}
              class="p-1 px-2 text-white bg-blue-500 border border-blue-600 rounded-sm hover:bg-green-500 focus:outline-none">Publish Report</button>
        </div>
      </div>
    ...
    <div class="flex-auto flex flex-row border shadow-xl z-20 {preview?'hidden':''}">
        ...
        <div class="absolute right-8 bottom-4 top-16 mt-2 flex-none w-96 flex flex-col border-r border-r shadow-2xl text-gray-500 bg-white z-20 {rcollapse?'hidden':''}">
          ...
          {#if selectedProp==="Dataset" }
              <Datasource 
                bind:reportData={reportObject} 
                on:datasourcechange={report_object_change}
                on:hiderighttoolbar={visibility_right_toolbar} 
                initiate={dsInitiate} />
          {/if}
        ...
        <!-- right toolbar -->
        <div class="flex-none w-8 flex flex-col z-20 {hideRightTools?'hidden':''}">
